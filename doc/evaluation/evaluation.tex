...
Together, these findings validate the use of a physics-informed U-Net architecture for spectral CT tissue decomposition 
and highlight its capacity to accurately resolve fine-grained differences in dual-energy attenuation profiles.


% \subsection{Interpretation}

% The results indicate that the hybrid loss function---which combines binary cross-entropy with a physics-informed 
% consistency term---effectively guides the network toward accurate and physically meaningful tissue segmentation. 
% The minimal error in tissue composition percentages suggests that the model not only captures image features but 
% also adheres to expected attenuation behavior governed by photoelectric and Compton interactions. The improved 
% performance of \texttt{UNet512} further highlights the benefit of deeper architectures for high-resolution CT data.
